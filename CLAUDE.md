...
> **This section used to carry a per-phase changelog** — every shipped item, its reasoning
> and its verification notes, ~1,100 lines. It was deleted on 2026-08-16: the *reasoning*
> is §6's job and the *shape of the app* is §4's, so the roadmap was a third copy of both.
> What is kept below is the shipping order (so the layering stays legible) and, more
> importantly, **every open item**, deduplicated across phases. Git history has the rest.

### What has shipped

| Phase | Landed | What it added |
|---|---|---|
| 0 | 2026-07-30 | Next.js + TS + Tailwind, Postgres on Railway, Prisma, first migration + seed |
| 1 | 2026-07-30 | Google auth (Auth.js v5, JWT, `AUTH_ALLOWLIST` fails closed), the app shell, the area tree |
| 2 | 2026-07-31 | **Studio** — brands, channels, series, content items, the stage board, batching, the daily queue |
| 3 | 2026-07-31 | **Tasks & the Hunt Board** — task CRUD, tracks, experiment capture, project tiering, project CRUD |
| 4 | 2026-08-01 | **Calendar** — hand-built month/week/day, events with recurrence, three switchable layers |
| 4.5 | 2026-08-02 | De-jargoning (`Mark→Task`, `Drop→ContentItem`), project pages, recurring tasks, `Doc` rows |
| 4.6 | 2026-08-04 | **The pressure came out** — the sprint retired, Today rebuilt project-first, `Project.focus`, the contribution map, the seed stopped creating work |
| 4.7 | 2026-08-05 | The Projects roster folded into Today's cards |
| 4.8 | 2026-08-05 | **Area pages** (`/areas/[slug]`), the **journal**, media in Postgres, `ProjectDoc → Doc` |
| 4.9–4.14 | 2026-08-05/06 | Real backlogs written for Sleepy Cat, Utaitai, Coding Mom and Forge; `Audio`, `Festivals` and `YC` tracks; task checklists (`Task.parentId`) |
| 4.13 | 2026-08-06 | **The dark theme**, following the sun (`lib/sun.ts`, `lib/theme-store.ts`) — see §11 |
| 4.15–4.17 | 2026-08-06 | The journal becomes a per-day thread; the camera, clips and colour grades; the media viewer |
| 4.18–4.19 | 2026-08-07 | A task opens where you read it; stage columns on the Tasks tab; `Brand.projectId` |
| 4.20–4.21 | 2026-08-08/09 | The phone's chrome hides on scroll; **it installs** (manifest, icons, service worker, offline page) |
| 5 | 2026-08-09 | **Montblanc** — a command bar in a drawer, DeepSeek hand-rolled, eight tools, receipts with Undo |
| 5.1–5.4 | 2026-08-09 | The camera goes full-bleed; a project keeps a journal; the composer gets a Markdown toolbar; the photo viewer swipes |
...
scripts** (`scripts/*-check.mts`, plus `ledger-smoke`, `ledger-live`, `ledger-reset`) holding
~300 assertions, because almost everything the Ledger does is arithmetic that is wrong
*silently*. Four real bugs came out of running them rather than reading them: a depreciation
tail that depended on the start month, a cursor that would have advanced past unwritten
transactions, an HSA counted as spendable cash, and a rule set that called itself usable with
empty bracket tables. **Run them after touching anything under `lib/tax/`, `lib/plaid*` or
`lib/money.ts`.**

### Known gaps

Deduplicated across every phase. Split into things genuinely undone, things deliberately not
built, and things unverified — the third list is the honest one, and it is mostly one sentence
repeated: *this has never been used on a real phone.*

**Not built yet, and wanted**

- [ ] **Area CRUD.** The last surface-level gap and the only noun with no editor — the
      sidebar's "Add area" and "Manage areas" have been disabled since Phase 1. Awkward now
      that an area has a page but cannot be created or renamed. Five areas ever, so it has
      still never actually bitten.
- [ ] **Captions on media.** `JournalMedia.caption` exists, is written as `null`, and has no
      UI on any of the three surfaces that show media. The full-screen viewer is the obvious
      place — it is the only screen with room.
- [ ] **Send the journal to her** — an email, a printed year, something. Deliberately not
      designed; the data is shaped for it (every entry dated, every photo addressable).
- [ ] **Multi-select delete on the Hunt Board.** Clearing 78 rows needed a script once.
- [ ] Drag to move / resize a calendar event. Every occurrence is already positioned from
      `startMinutes`, so this is a pointer handler and a server action, not a rewrite.
- [ ] Per-occurrence calendar exceptions ("skip *this* one"). Needs a table.
- [ ] Per-project content item list on the project card.
- [ ] **`TaxProfile` constants.** Not one tax constant is confirmed, so the Tax tab computes
      nothing **by design** (§6). Filling them in is Layer 7's draft flow plus a person and
      an afternoon.
- [ ] **The Docs editor has no Markdown toolbar**, and it is the more Markdown-heavy of the
      two surfaces. `components/ui/markdown-toolbar.tsx` takes any textarea ref — four lines.
- [ ] **Nothing shows that a task has comments.** `_count.comments` on a query already
      running, plus a badge on three deliberately one-line card layouts.
- [ ] **Nothing shows a Montblanc note's source conversation.** `sourceId` is written and the
      log is kept; no screen joins them, so "why do you think that" is a database query.
- [ ] **Montblanc has no surface-aware context** — it does not know which screen you are on.
      One field on the request, deliberately not guessed at before use.
- [ ] Reordering checklist steps, and reordering media. Both arrive in the order written,
      which for a list of accounts is the order you open them in.
- [ ] **Tables in a doc render as raw pipes.** `lib/markdown.ts` has never supported them and
      `forge-vision.md` has two. The only known gap in the renderer.
- [ ] Enter on a bulleted line does not continue the list.
- [ ] **No quarterly recurrence**, so "apply to the next YC batch" cannot be a repeating row —
      the enum is daily/weekdays/weekly/monthly.
- [ ] **A custom domain.** The other half of "it installs", and it is a Railway setting plus a
      DNS record, not code. See `docs/install.md` for the `AUTH_URL` change that goes with it.
- [ ] **Push notifications.** The service worker has no `push` handler; that is Phase 7, and
      on iOS it requires the app to have been installed first — which it now can be.

**Deliberately not built** _(revisit only if the reason changes)_

- [ ] **Montblanc cannot edit or delete**, beyond ticking a task off and undoing its own
      writes. It is a way *in*, not a way to rearrange. §6.
- [ ] **The Montblanc transcript is not kept between opens.** Each open is a fresh sheet.
- [ ] **Proactive help** (daily briefing, drift nudges) — the half most likely to reproduce
...
- [ ] **No recurring posting task for Coding Mom.** A daily row nagging about an account that
      ...
      what creates it.
- [ ] **Xiaohongshu is absent** — it is where the Chinese-learning audience actually is, and
      it needs a `Platform` value plus a Chinese phone number.
- [ ] **Forge's borrowing of Coding Mom's audience is not modelled.** It is expressed per
      item; a `Project.audienceBrandId` would be a third place for the same fact to disagree.
- [ ] **Nothing links a channel to the tasks about creating it**, or the Next Fest track to
      its event. Same shape both times, and both are smaller than a table.
- [ ] **AMT, K-1s, 1031 exchanges, a mid-year move and most state credits are not modelled**,
      and are enumerated permanently on the Tax tab rather than approximated.
- [ ] **Reconciliation cannot catch a mislabelled statement row.** A repair booked as
      insurance adds up perfectly and is wrong on the Schedule E — hence no auto-accept, and
      `rawText` on every row.

**Unverified** _(written and never exercised — treat as unproven, not as working)_
...
- [ ] **The Ledger has never been looked at in a browser at all.** The Chrome extension has no
      host permission for `localhost:3100`, so all eight layers are verified by build,
      typecheck, lint, nine check scripts and curl — and not by a screenshot.
- [ ] **No frame has ever been captured from a real camera.** Granting Chrome's camera
      permission is the user's call. Everything around the capture is exercised against a
      synthetic feed; the lens is not. A clip's real length is unexercised for the same
      reason (§9's occluded-window limit).
- [ ] **Plaid Production** is an application with a review and a per-bank monthly bill.
      Everything is built against Sandbox.
- [ ] **Gmail has never been connected** — the grant flow, the poll and the attachment decode
      are written and unexercised. Needs a consent screen and a second redirect URI.
- [ ] **RentCast has never been called** — no `RENTCAST_API_KEY` yet.
- [ ] **The Plaid webhook has never received a real delivery** (needs a public URL). Safe: the
      read-triggered catch-up makes it an optimisation, never a dependency.
- [ ] **The tax-rules updater has never fetched a real page.** First honest test is October.
- [ ] **The estimate has never run on confirmed constants**, because none are confirmed.
- [ ] **The statement extractor has only seen a test PDF with a standard font.** A real
      AppFolio statement embeds a subset font — exactly the case §8 said a hand-written
      extractor could not handle, and exactly what the fixture cannot prove.
- [ ] **The loan-address matcher has never seen a real servicer string** (0.67 on a synthetic).
- [ ] **Montblanc's Ledger tools have not been driven through the drawer against the live
      model** — exercised at the schema, registry and database level only.
- [ ] **Montblanc's memory has only ever been distilled from staged conversations.** Nothing
      here has met a week of real use, which is the only thing that shows whether the notes
      earn their place in the prompt. Distillation is also **unbounded in cost**: one model
      call per conversation, unseen and unaccounted.
- [ ] **Server action wrappers are exercised only in a browser** — the auth check, the
      `revalidatePath` and the write ordering. §9 explains why a script cannot reach them.

**Standing data caveats** _(true of the content, not the code)_

- [ ] **Every festival deadline and YC batch date is unverified**, recorded in notes rather
      than asserted as a due date. §6, "An unverified date is a note, not a due date".
- [ ] **The October 2026 Next Fest dates are a guess** — Mon 12 – Mon 19, following the 2024
      and 2025 pattern. The calendar event and every downstream date assume it.
- [ ] **Utaitai's lifetime pricing conflicts with its own goal** — a lifetime purchase
      contributes $0 MRR by definition. On the board as a decision, not a build.
- [ ] **The old Utaitai paywall's conversion rate is unrecorded**, and the window closes the
      moment it is switched off. The one dated row on that project.
- [ ] **Due dates exist on almost nothing**, deliberately — §6, "A goal with no deadline gets
      no due dates".
- [ ] **Coding Mom reads "Drifting"** on `cadenceDays: 1` while its first task is still to
      create an e-mail account. Correct the day posting starts, wrong until then.
...
- [x] **Calendar library** — resolved 2026-08-01: **neither. Hand-built.** Month is a
      ...
      drag-to-move doesn't come for free; see "Known gaps" in §5.
...
**The through-line, if you read nothing else here.** Every hard-won rule in §6 is the same
refusal in a different costume: **the app must not assert things nobody told it.** No
seeded tasks, no invented events, no guessed due dates, no tax constant from memory. A
cursor is not advanced past a row that was not written. A statement is not accepted until
it reconciles to the cent. A property with no valuation contributes nothing rather than
its purchase price. An incomplete Schedule E withdraws the whole estimate. A strategy is a
question, never an instruction. The cost of getting this wrong is asymmetric and that is
the whole argument: a row you wrote and no longer want costs one tap to delete; a row **you
did not write** costs you a stop, a re-read, and a decision about whether it was yours.

**What is genuinely not done is mostly not code.** Plaid Production is an application and a
bill; Gmail needs a consent screen and a redirect URI; RentCast needs a key; the tax
constants need a person and an afternoon. See "Known gaps" in §5.

> **Note on this file.** It was a running changelog until 2026-08-16, when ~2,400 lines of
> per-phase retrospectives were deleted — every one of them re-narrated a decision already
