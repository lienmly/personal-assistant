    ...
    <>
      <div className="mb-5 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 rounded-chip bg-accent px-3.5 py-2 text-[13px] font-medium text-white hover:bg-accent-hover"
        >
      ...
      {goingOutToday > 0 && (
        <p className="mb-4 inline-flex items-center gap-2 rounded-chip bg-accent-soft px-3 py-1.5 text-[13px] text-accent">
        ...
        <div className="flex min-w-max gap-4">
          {STAGES.map((stage) => {
            ...
            return (
              <div
                key={stage.id}
                className="flex w-[248px] flex-col rounded-card bg-card p-3.5 shadow-card"
              >
                ...
                <div className="flex min-h-[180px] flex-1 flex-col gap-2">
                  ...
                  {column.length === 0 && (
                    <button
                      type="button"
                      onClick={() => setPanel({ mode: "new" })}
                      className="flex flex-1 items-center justify-center rounded-tile border border-dashed border-line text-faint hover:text-muted"
                    >
...
function FilterChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 rounded-chip px-3 py-2 text-[13px] transition-colors",
        active
...
function DropCard({ drop, onOpen }: { drop: DropView; onOpen: () => void }) {
  ...
  return (
    <div
      className={cn(
        "group rounded-tile bg-inset p-3 text-left transition-colors hover:bg-inset/70",
    ...
    >
      ...
      {next && (
        <button
          ...
          className="mt-2 flex w-full items-center justify-center gap-1 rounded-chip py-1 text-[11px] text-faint opacity-0 transition-opacity hover:bg-card hover:text-ink group-hover:opacity-100 disabled:opacity-40 max-md:opacity-100"
        >
          {next.label}
